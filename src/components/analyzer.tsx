...
import {
	...
	type GenerateIntermediateCodeOutput,
} from "@/ai/flows/generate-intermediate-code-flow";
import {
	analyzeSyntax,
	type AnalyzeSyntaxOutput,
} from "@/ai/flows/analyze-syntax-flow";
import {
	analyzeSemantics,
	type AnalyzeSemanticsOutput,
} from "@/ai/flows/analyze-semantics-flow";
...
export function Analyzer() {
	...
	const [intermediateCode, setIntermediateCode] =
		useState<GenerateIntermediateCodeOutput | null>(null);
	const [syntaxAnalysis, setSyntaxAnalysis] =
		useState<AnalyzeSyntaxOutput | null>(null);
	const [semanticAnalysis, setSemanticAnalysis] =
		useState<AnalyzeSemanticsOutput | null>(null);

	const [isLexicalLoading, setIsLexicalLoading] = useState<boolean>(false);
	const [isMachineCodeLoading, setIsMachineCodeLoading] =
		useState<boolean>(false);
	const [isIntermediateCodeLoading, setIsIntermediateCodeLoading] =
		useState<boolean>(false);
	...
	const handleAnalyzeLexical = useCallback(
		async (showToasts = true): Promise<boolean> => {
			if (!code.trim()) {
				if (showToasts)
					toast({
			...
			try {
				...
				const { tokens, symbolTable, lexemeStats, tac } = analyzeCode(
					code,
					language
				);
				setTokens(tokens);
				...
				if (showToasts)
					toast({
						...
						description: `${
							error instanceof Error ? error.message : "Check console."
						}`,
						variant: "destructive",
					});
				setTokens([]);
	...
	const handleGenerateMachineCode = useCallback(
		async (showToasts = true): Promise<boolean> => {
			if (!code.trim()) {
				if (showToasts)
					toast({
			...
			try {
				...
				if (showToasts)
					toast({
						...
						description: `Successfully generated instructions.`,
					});
				return true;
			} catch (error) {
	...
	const handleGenerateIntermediateCode = useCallback(
		async (showToasts = true): Promise<boolean> => {
			if (!code.trim()) {
				if (showToasts)
					toast({
			...
			try {
				...
				if (showToasts)
					toast({
						...
						description: `Successfully generated IC representations.`,
					});
				return true;
			} catch (error) {
	...
	const handleAnalyzeSyntax = useCallback(
		async (showToasts = true): Promise<boolean> => {
			if (!code.trim()) {
				if (showToasts)
					toast({
						title: "Input Required",
						description: "Please enter code for syntax analysis.",
						variant: "destructive",
					});
				return false;
			}
			...
			try {
				...
				if (showToasts)
					toast({
						title: "Syntax Analysis Complete",
						description: result.parseStatus,
					});
				return true;
			} catch (error) {
		...
		async (showToasts = true): Promise<boolean> => {
			if (!code.trim()) {
				if (showToasts)
					toast({
						title: "Input Required",
						description: "Please enter code for semantic analysis.",
						variant: "destructive",
					});
				return false;
			}
	...
	const handleAnalyzeAll = useCallback(async () => {
		...
		toast({
			...
			description: "Processing all phases...",
		});

		let success = await handleAnalyzeLexical(false);
		if (!success) {
			toast({
				title: "Full Analysis Halted",
				description: "Lexical analysis failed.",
				variant: "destructive",
			});
		...
		toast({
			title: "Lexical Analysis Complete",
			description: "Proceeding to Syntax Analysis.",
		});
		...
		if (!success) {
			toast({
				title: "Full Analysis Halted",
				description: "Syntax analysis failed.",
				variant: "destructive",
			});
			setIsAllLoading(false);
			return;
		...
		toast({
			title: "Syntax Analysis Complete",
			description: "Proceeding to Semantic Analysis.",
		});
		...
		toast({
			title: "Semantic Analysis Complete",
			description: "Proceeding to Intermediate Code Generation.",
		});
		...
		if (!success) {
			toast({
				title: "Full Analysis Halted",
				description: "Intermediate Code generation failed.",
				variant: "destructive",
			});
			setIsAllLoading(false);
			return;
		...
		toast({
			title: "Intermediate Code Generation Complete",
			description: "Proceeding to Machine Code Generation.",
		});
		...
		if (!success) {
			toast({
				title: "Full Analysis Halted",
				description: "Machine Code generation failed.",
				variant: "destructive",
			});
			setIsAllLoading(false);
			return;
		...
		toast({
			...
			description: "All phases processed successfully.",
		});
		setIsAllLoading(false);
	}, [
		code,
		...
		handleGenerateIntermediateCode,
		handleGenerateMachineCode,
	]);
	...
	const handleReset = useCallback(() => {
		...
		toast({
			...
			description: "Editor and results cleared. Sample code loaded.",
		});
	}, [language, toast]);
	...
	const handleUseSample = useCallback(() => {
		...
		toast({
			...
			description: `Loaded sample ${language.toUpperCase()} code.`,
		});
	}, [language, toast]);
	...
	const handleLanguageChange = (value: string) => {
		...
		toast({
			...
			description: `Switched to ${newLang.toUpperCase()}. Sample code loaded and results cleared.`,
		});
	};

	const anyIndividualLoading =
		isLexicalLoading ||
		isMachineCodeLoading ||
		isIntermediateCodeLoading ||
		isSyntaxLoading ||
		isSemanticsLoading;
					...
					<div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
						...
						<div className="flex gap-2 flex-wrap justify-end">
							...
							<Button
								...
								disabled={anyLoading || !code.trim()}
								className="bg-gradient-to-r from-pink-500 to-rose-500 text-white hover:from-pink-600 hover:to-rose-600 shadow-md"
							>
							...
							<Button
								onClick={() => handleGenerateIntermediateCode()}
								disabled={anyLoading || !code.trim()}
								className="bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 shadow-md"
							>
								<Shuffle className="mr-1 h-4 w-4" />{" "}
								{isIntermediateCodeLoading ? "Gen IC..." : "Gen IC"}
							</Button>
							<Button
								onClick={() => handleGenerateMachineCode()}
								disabled={anyLoading || !code.trim()}
								className="bg-gradient-to-r from-lime-500 to-green-500 text-white hover:from-lime-600 hover:to-green-600 shadow-md"
							>
							...
							<Button
								...
								size="icon"
								disabled={anyLoading}
								className="border-destructive text-destructive hover:bg-destructive/10 shadow-sm"
							>
